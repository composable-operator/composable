...
var _ = BeforeSuite(func() {
	...
	testEnv = &envtest.Environment{
		CRDDirectoryPaths: []string{
			filepath.Join("..", "config", "crd", "bases"), // Operator CRDs
			filepath.Join("./testdata", "crds"),           // Test CRDs
		},
	...
	err = ibmcloudv1alpha1.AddToScheme(scheme.Scheme)
	Expect(err).NotTo(HaveOccurred())
	...
	const testNS = "tmp-test-composable"
	testContext = test.NewTestContext(k8sManager.GetClient(), testNS)
	test.CreateNamespaceOrDie(ctx, testContext, testNS)
}, 60)
